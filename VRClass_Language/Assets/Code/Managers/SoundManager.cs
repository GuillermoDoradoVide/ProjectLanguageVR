...
public class SoundManager : SingletonComponent<SoundManager>
{
    ...
    {
        audioSourceCollection = null;
        audioSourceCollection = FindObjectsOfType<AudioSource>();
    }
    ...
    public void stopMusicAndSounds(bool fade, float fadeDuration = 1f)
    {
        SoundManager manager = Instance;
        if (manager.audioSourceCollection != null)
        {
            if (fade)
            {
                StartCoroutine(Instance.fadeAllSounds(0.0f, fadeDuration));
            }
            else
            {
                for (int i = 0; i < manager.audioSourceCollection.Length - 1; i++)
                {
                    manager.audioSourceCollection[i].Stop();
                }
            }
        }
    }
    ...
    public static void disableSoundImmediate()
    {
        ...
        if (manager.audioSourceCollection != null)
        {
            for (int i = 0; i < manager.audioSourceCollection.Length - 1; i++)
            {
                manager.audioSourceCollection[i].volume = 0;
            }
        }
    ...
    public static void enableSoundImmediate()
    {
        ...
        if (manager.audioSourceCollection != null)
        {
            for (int i = 0; i < manager.audioSourceCollection.Length - 1; i++)
            {
                manager.audioSourceCollection[i].volume = getSFXVolume();
            }
        }
        manager.musicSource.volume = getMusicVolume();
        isMuted = false;
